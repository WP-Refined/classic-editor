  ...
  <div class="dt-w-1/2 sm:dt-w-full overflow-hidden">
    <data-table :rows="tableData"
      ...
      <template #tbody="{row}">
        ...
        <table-body v-text="truncateText(formatRenderedContent(row.excerpt.rendered), 50)"/>
...
<script>
  ...
  const settings = reactive({
    apiUrl: 'https://stayathomemum.com.au', // wp-preflight.local
  });
  ...
  export default {
    ...
    setup() {
      ...
      const loadData = async (query) => {
        ...
        const response = await axios.get(settings.apiUrl + '/wp-json/wp/v2/posts', {
          params: {
        ...
        pagination.value = { page: query.page, total: Number(response.headers['x-wp-totalpages']) };
      ...
      const formatDate = (date) => date ? DateTime.fromISO(date).toFormat('FF') : null;

      const formatUrl = (url) => url.startsWith("http") ? url : `https://${url}`;

      const truncateText = (text, max) => {
        const truncated = text && text.length > max
